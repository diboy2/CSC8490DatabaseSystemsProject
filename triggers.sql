...
CREATE OR REPLACE  TRIGGER finalize_order AFTER UPDATE ON RESERVATION
FOR EACH ROW
BEGIN
    IF :NEW.PAYMENT_DETAILS_ID IS NOT NULL AND :NEW.DELIVERY_ID IS NOT NULL THEN
      :NEW.RESERVATION_STATUS := '''COMPLETE''';
    END IF;
END;
/
