...
DECLARE
    grand_total FLOAT
BEGIN
    SELECT SUM(SHOPPING_CART.QUANTITY * F.FOOD_PRICE) AS grand_total
        FROM SHOPPING_CART
        JOIN FOOD AS F
        ON F.FOOD_ID = SHOPPING_CART.FOOD_ID
        WHERE SHOPPING_CART_ID = :NEW.SHOPPING_CART_ID;
    INSERT INTO RESERVATION(SHOPPING_CART_ID, NET_AMOUNT, TOTAL_AMOUNT, ORDER_TIME, RESERVATION_STATUS, DELIVERY_ID, PAYMENT_DETAILS_ID)
    VALUES(:NEW.SHOPPING_CART_ID, grand_total, grand_total * 1.06, CURRENT_TIMESTAMP, "SUBMITTED", NULL, NULL);
END;
/
